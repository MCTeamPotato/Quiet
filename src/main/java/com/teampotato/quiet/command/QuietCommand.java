...
public class QuietCommand {
    ...
    public static int quietPlayer(CommandSourceStack commandSource, Collection<ServerPlayer> players, byte strength, int time) {
        ServerLevel serverLevel = commandSource.getLevel();
        players.forEach(serverPlayer -> {
            QuietData quietData = new QuietData(serverPlayer.getScoreboardName(), serverPlayer.getUUID(), strength, serverLevel.getGameTime() + time);
            QuietDataUtil.addQuietData(quietData);
            NetworkHandler.CHANNEL.send(
                    PacketDistributor.PLAYER.with(() -> serverPlayer),
                    new QuietDataS2C(quietData)
            );
            if (strength >= 3) serverPlayer.sendSystemMessage(Component.translatable("message.quiet.mute"));
        });
        return 0;
    }
}
